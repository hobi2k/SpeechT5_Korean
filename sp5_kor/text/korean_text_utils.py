...
분리 이유
- 훈련 데이터는 대개 음성-텍스트 페어로 “이미 녹음된 발화”에 대응됩니다.
  여기서 텍스트를 과하게 변형(예: 숫자 1203 -> “천이백삼”)하면,
  원래 녹음에서 실제로 말한 방식(“천이백삼”인지 “일이공삼”인지 등)과 불일치하여
...
# 추론 시 “숫자를 읽어주는 TTS”를 만들려면 이런 규칙 기반 변환이 필요합니다.
_NUMBER_KOR = {
    0: "영",
    1: "일",
    2: "이",
...
def inject_tokens_for_training(text: str) -> str:
    ...
    # 이 단계는 토크나이저가 ‘실제 문자’를 배울 필요 없이 의미 토큰만 보면 되게 해줍니다.
